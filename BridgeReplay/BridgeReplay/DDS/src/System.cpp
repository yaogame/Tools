...
#ifdef __APPLE__
  // BridgeReplay: iOS 不允许 popen，改用 sysctlbyname 读取物理内存。
  #include <sys/types.h>
  ...
  unsigned long long& kilobytesFree) const
{
  ...
  // BridgeReplay: popen("sysctl -n hw.memsize") replaced by sysctlbyname
  // because popen is unavailable on iOS.
  unsigned long long memsize = 0;
  size_t memsizeLen = sizeof(memsize);
  if (sysctlbyname("hw.memsize", &memsize, &memsizeLen, NULL, 0) == 0)
    kilobytesFree = memsize;
  else
    kilobytesFree = 1024ULL * 1024 * 1024; // guess 1GB
